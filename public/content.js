...
chrome.runtime.onConnect.addListener(function (p) {
    ...
    p.onMessage.addListener(function(msg) {
        if(msg.type === "requestState") {
            ...
            p.postMessage({ type: "state", brightness: (1 - tag.style.opacity) * 100, persist });
        }
        if(msg.type === "updateBrightness") {
            updateBrightness(msg.newValue);

            if(msg.source === undefined && persist) {
                chrome.runtime.sendMessage({ type: msg.type, value: msg.newValue, persist }, function responseCallback() {});
            }
        }
